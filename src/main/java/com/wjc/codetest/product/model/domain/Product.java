 ...
 *          이와 같은 경우 개발자가 10개의 로직을 직접 확인하여 문제가 되는 부분을 찾아야 하는 비효율적인 작업을 진행해야 합니다.
 *
 *          또한 단순히 setName() 이라는 명칭은 name이라는 필드에 값을 대입한다 이상의 의미를 갖지 못합니다.
 *          * 예를 들어 상품명 앞에 '[변경됨]'을 붙여야 하는 요구사항이 있다면, setName() 보다는 changeProductName()과 같이
 *            비즈니스 행위가 명확히 드러나는 메서드를 정의하는 것이 좋습니다.
 *
 *          따라서 @Setter 어노테이션을 제거하여 비즈니스와 상관 없이 필드의 값을 변경할 수 없도록 닫고
 *          비즈니스에 맞는 명확한 명칭으로 역할을 수행하는 메서드들을 구현하는 것이 좋을 것 같습니다.
 */
@Setter
public class Product {
     ...
     *          클래스 레벨에 아래와 같이 작성하고 아래의 기본 생성자는 삭제하는 것이 좋을 것 같습니다.
     *
